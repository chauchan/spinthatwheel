...
static func get_starter_support() -> Array[CardData]:
	var cards: Array[CardData] = []
	cards.append(_make("Company Policy", CardData.CardType.SUPPORT, 1, 0, 0, "Takes -2 damage next time you get berated", false, CardData.EffectDuration.PERSISTENT))
	...
	cards.append(_make("Loan", CardData.CardType.SUPPORT, 0, 0, 0, "+3 energy, -1 energy next 2 turns", false, CardData.EffectDuration.PERSISTENT))
	cards.append(_make("Investing", CardData.CardType.SUPPORT, 0, 0, 0, "+1 energy this turn, +2 energy after 2 turns", false, CardData.EffectDuration.PERSISTENT))
	cards.append(_make("Savings Account", CardData.CardType.SUPPORT, 0, 0, 0, "Gain energy for each card you have in your hand", false))
	return cards


static func _make(n: String, t: CardData.CardType, c: int, p: int, h: int, d: String, ar: bool = true, ed: CardData.EffectDuration = CardData.EffectDuration.INSTANT) -> CardData:
...
static func get_all_support() -> Array[CardData]:
	var cards: Array[CardData] = []
	cards.append_array(get_starter_support())
	return cards
